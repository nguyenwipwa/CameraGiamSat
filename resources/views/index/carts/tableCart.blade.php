<table id="cart" class="hidden-xs hidden-sm">
	...
	<form id="formThanhToan" action="{{ route('payment1') }}" method="post" accept-charset="utf-8">
		<tbody>
			<input type="hidden" name="id_user" value="{{ $user->id }}">
			<tr>
				<td colspan="2">
					<span class="cart_3_header">Hoàn tất đơn hàng</span>
					<input id="code_order" type="hidden" name="code_order" value="MDH_{{ time() }}">
				</td>
			</tr>
			<tr>
				<td style="width:40%;">
					...
					<div class="form_label_input">
						...
						<span><input class="form-control" type="text" readonly="readonly" name="fullname" value="{{ $user->name }}" /></span>
					</div>
					<div class="form_label_input">
						<label style="font-weight:bold" for="phone">Điện thoại  <span class="required"> * </span></label>
						<span><input readonly="readonly" class="form-control" type="text" name="phone" value="{{ $user->phone_number }}"  /></span>
					</div>
					<div class="form_label_input">
						<label style="font-weight:bold" for="email">Email <span class="required"> * </span></label>
						<span><input readonly="readonly" class="form-control" type="text" name="email" value="{{ $user->email }}"  /></span>
					</div>
					<div class="form_label_input"><label style="font-weight:bold" for="address">Địa chỉ <span class="required"> * </span></label>
						<span>
							<input readonly="readonly" class="form-control" type="text" name="address" value="{{ $user->address->address }}"  /></span>
						</div>
						<div class="form_label_input"><label style="font-weight:bold" for="address">Thành phố <span class="required"> * </span></label>
							<span>
								<input readonly="readonly" class="form-control" type="text" name="thanhpho" value="{{ $user->address->thanhpho->name }}"  /></span>
							</div>
								...
								<table id="cart" class="inside">
									<tbody>
										...
										<tr id="giamgia" class="giamgia" style="display: none;">
											...
											<td class="text_right" colspan="1">
												<span id="tiengiam" class="tiengiam" style="display:block;color:blue;font-weight:bold;">{{ Cart::total() }} VNĐ</span>
											</td>
										</tr>
										<tr>
			...
			<tbody>
				<form id="formThanhToanTiny" action="" method="post" accept-charset="utf-8"><tr>
					<td>
						<span class="cart_3_header">Hoàn tất đơn hàng</span>
						<input type="hidden" name="id_user" value="{{ $user->id }}">
						<input id="code_order1" type="hidden" name="code_order" value="MDH_{{ time() }}">
					</td>
				</tr>
				<tr>
					<td style="width:40%;">
						...
						<script type="text/javascript">
							...
							var priceNganLuong =  {{ Cart::total1() }};
					...
					success: function(data) {
						if(data.status=="error"){
							...
							var linka = "https://www.nganluong.vn/button_payment.php?receiver=trongitnlu@gmail.com&product_name="+code_order+"&price="+({{ Cart::total1() }} - tien)+"&return_url=google.com&comments="+tenKhacHang+"";
							$('.nganluong').attr('href', linka);
							$('#pricePaypal').val({{ Cart::total1() }} - tien);
							$('#apaypal').attr('onclick', "openRequestedPopup('{{url('gio-hang/paypal')}}/"+code_order+"/"+({{ Cart::total1() }} - tien)+"')");
						}
					}
				});
							}
