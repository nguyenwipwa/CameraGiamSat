...
class PageController extends Controller {
	...
	function detailProduct($id) {
		...
		$provider = DB::select('SELECT name from provider where id = ?', [$id]);
		return view('index.product.chitietsanpham', ['category' => $category, 'contact' => $contact, 'product' => $product, 'provider' => $provider]);
		// var_dump($product);
		// echo $id;
	}
	//
